...
import { Tree, Empty, Radio, RadioChangeEvent } from 'antd';
...
const APIPage: React.FC<APIPageProps> = ({ data }) => {
  const apis = data.api.edges;
  const guides = data.guide.edges;
  const [selectTab, setSelectTab] = React.useState('api');
...
export const query = graphql`
  query MyQuery {
    ...
    api: allMarkdownRemark(
      filter: { fileAbsolutePath: { regex: "/utils/docs/api/" } }
    ) {
      edges {
        node {
          frontmatter {
    ...
    guide: allMarkdownRemark(
      filter: { fileAbsolutePath: { regex: "/utils/docs/guide/" } }
    ) {
      edges {
        node {
          frontmatter {
